...
import { addTobestsellers, addTobrand_bestsellers, addToHits, changeCount, getAllOrders, getAllProducts, getBrands, getOrders } from '../API'
...
export default class Products extends React.Component {
  ...
  componentDidMount(){
    getAllProducts(this, 1);
    getBrands(this);
  }

  filterOrers(status){
    getAllProducts(this, status);
    getBrands(this);
  }

  addToHit(id, hitStatus, api){
    addToHits(this, id, hitStatus);
    getAllProducts(this, 1);
  }

  addTobestsellers(id, hitStatus){
    addTobestsellers(this, id, hitStatus);
    getAllProducts(this, 1);
  }

  addTobesd_bestsellers(id, hitStatus){
    addTobrand_bestsellers(this, id, hitStatus);
    getAllProducts(this, 1);
  }
        ...
        <div className="app-content pt-3 p-md-3 p-lg-4">
          <div className="container-xl">
            <div className="row g-3 mb-4 align-items-center justify-content-between">
              <div className="col-auto">
                <h1 className="app-page-title mb-0">Товары</h1>
              </div>
              <div className="col-auto">
           ...
           aria-controls="orders-all"
            ...
            {this.state.brand == null ? <div className="tab-content" id="orders-table-tab-content">
              <div
                      ...
                      <table className="table app-table-hover mb-0 text-left">
                        ...
                        <tbody>
                         {this.state.brands.map((item) => (
                           ...
                           <td>
                            {this.state.orders.filter((item2) => item2.brand?.id == item.id).length}

                           </td>
                          

                           <td className="cell">
                             <a onClick={() => this.setState({brand: item.id})} className="btn-sm app-btn-secondary" href="#">
                               View
                             </a>
                           </td>
                         </tr>
            ...
            </div>: <div className="tab-content" id="orders-table-tab-content">
              <div
                className="tab-pane fade show active"
                id="orders-all"
                role="tabpanel"
                      ...
                      <table className="table app-table-hover mb-0 text-left">
                        <thead>
                          <tr>
                            ...
                            <th className="cell">Hits</th>
                          
                            <th className="cell">Brand bestsellers</th>
                            <th className="cell">Bestsellers</th>
                              <th className="cell">Count</th>
                            <th className="cell" />
                          </tr>
                        </thead>
                        <tbody>
                         {this.state.orders.filter((item2) => item2.brand.id == this.state.brand).map((item) => (
                           <tr>
                           <td className="cell">#{item.id}</td>
                           <td className="cell">
                             <span className="truncate">
                           ...
                           <td className="cell">{item.product_type_id?.title_ru}</td>
                           <td className="cell">
                             <span>{item.brand?.name}</span>
                             {/* <span className="note">2:16 PM</span> */}
                           </td>
                           <td className="cell">
                             <span className={"badge bg-success"}>{item.stock}</span>
                           </td>
                           <td className="cell">{item.price}</td>
                           <td className="cell"><input type="checkbox" checked={item.hit} onChange={(e) => this.addToHit(item.id, item.hit == true ? false : true)} /></td>
                           <td className="cell"><input type="checkbox" checked={item.bestsellers} onChange={(e) => this.addTobestsellers(item.id, item.bestsellers == true ? false : true)} /></td>
                           <td className="cell"><input type="checkbox" checked={item.brand_bestsellers} onChange={(e) => this.addTobesd_bestsellers(item.id, item.brand_bestsellers == true ? false : true)}  /></td>
                           <td className="cell"><input type="text" style={{width: '80px'}} placeholder={item.stock} onChange={(e) => this.changeStock(item.id, e.target.value)}  /></td>
                           <td className="cell">
