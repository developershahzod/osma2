...
import { getAllOrders, getAllProducts, getClients, getOrders } from '../API'
...
export default class Clients extends React.Component {
  ...
  handleSend = async (e) => {
    ...
    try {
      // Шаг 1: Получаем всех пользователей (токены)
      const response = await axios.get('https://osma.academytable.ru/api/v1/notif_all/');
      const tokens = response.data;

      // Шаг 2: Отправляем каждому пуш
      for (const item of tokens) {
        await axios.post('https://osma.academytable.ru/fcm/send-push/', {
          token: item.token,
      ...
      this.setState({ status: '✅ Уведомления отправлены!', loading: false });

    } catch (error) {
      console.error(error);
      this.setState({ status: '❌ Ошибка при отправке', loading: false });
    }
  };

  render() {
    const { title, body, status, loading } = this.state;
        ...
        <div className="app-content pt-3 p-md-3 p-lg-4">
          <div className="container-xl">
            <div className="row g-3 mb-4 align-items-center justify-content-between">
              <div className="col-auto">
                <h1 className="app-page-title mb-0">Клиенты</h1>
              </div>
              <div className="col-auto">
            ...
            <div>
              <h5>Отправить всем пуш уведомления</h5>
                      ...
                      <table className="table app-table-hover mb-0 text-left">
                        ...
                        <tbody>
                         {this.state.orders.map((item) => (
                           <tr>
                           <td className="cell">
                            <NavLink to={'/order_user/' + item.id + '/' + item.name + ' ' + item.surname  + '/'}>
                            #{item.id}
                            </NavLink>
                            </td>
                          ...
                          {item.address}
                           ...
                           <td>{this.state.orders_all.filter((item2) => item2.user_id.id == item.id).length}</td>
                           <td className="cell">
